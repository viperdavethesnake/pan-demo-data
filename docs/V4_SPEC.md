...
24. **2026-04-20 (v4.1.0)**: **Folder density expansion** — first 10M run took 9h49m because heavy-tail landed on mega folders (415K / 365K / 307K files each). NTFS directory insert cost jumps ~20x when a single directory exceeds 100-200K entries, and heavy-tail's Ultra bucket (1% × 500K max) guaranteed this. Fix has two halves: **(a)** cap Ultra at 100K (was 500K) in `HeavyTailDistribution`; **(b)** expand the folder tree so files distribute across more directories. Added folder classes (all config-driven, per-dept):
    - `Sales/Clients/<Client>/{Contracts,Proposals,Invoices,Correspondence,Projects}` — 20-40 per Sales
    - `Legal/Matters/<Matter>/{Pleadings,Discovery,Briefs,Correspondence,Evidence}` — 15-25 per Legal
    - `Procurement|Finance/Vendors/<Vendor>/{Contracts,POs,Invoices,Statements}` — 20-30 / 10-20
    - `Marketing/Campaigns/<Campaign>/{Brief,Assets,Reports,Email}` — 15-25 per Marketing
    - `IT/Apps/<App>/{Logs,Configs,Releases}` — 12-20 per IT
    - `Archive/<year>/{Q1,Q2,Q3,Q4}` everywhere (was just `<year>`)
    - User home dirs: `DeptUserCount = $null` (all real users, was capped at 12); `RootFraction = 1.0` (all users get root home too, was 0.4)
    - Project sub-folders: now deterministic (all 5 per project), was 33% chance each
    Data pools expanded: Clients 30→60, Matters 10→30, new Apps pool (20), new Campaigns pool (25). New name-template patterns for each new folder class.
    Projected outcome: folder count goes from ~820 to ~2,400-2,700. Average files per folder drops from 12K to ~4K. No single folder exceeds ~100K. Expected 10M runtime: **~5 hours** (vs 9.8 before). Tagged `v4.1.0`. Smoke not yet re-run at this configuration — pending verification before a full-scale rebuild.
